//! FASTA parsing for template and reference sequences.
...
impl ReferenceData {
    pub fn new() -> Self {
        Self { sequences: Vec::new(), names: Vec::new() }
    }
...
mod tests {
    ...
    #[test]
    fn test_parse_template_rna_converts_u_to_t() {
        let fasta = ">RNA_Template\nACGUACGU";
        let data = parse_template_fasta(fasta).unwrap();
        assert_eq!(data.sequence, "ACGTACGT");
    }

    #[test]
    fn test_parse_references_rna_converts_u_to_t() {
        let fasta = ">Ref1\nACGUACGU\n>Ref2\nUUUACG";
        let data = parse_reference_fasta(fasta).unwrap();
        assert_eq!(data.sequences[0], "ACGTACGT");
        assert_eq!(data.sequences[1], "TTTACG");
    }

    #[test]
    fn test_parse_rna_lowercase_u() {
        let fasta = ">Seq\nacguacgu";
        let data = parse_template_fasta(fasta).unwrap();
        assert_eq!(data.sequence, "ACGTACGT");
    }
